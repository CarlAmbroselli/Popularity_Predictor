...
class Word2VecMore(Features):
    ...
    def initialize_variables(self, df):
        # load model
        language = 'de'
        filepath = 'model/word2vec/all_lowercased_stemmed_' + str(self.num_dimensions) + '_' + language + '_' + str(self.window_size)
        if os.path.isfile(filepath):
            ...
            self.w2v_model = gensim_word2vec(texts, size=self.num_dimensions, window=self.window_size, min_count=5, workers=10)
